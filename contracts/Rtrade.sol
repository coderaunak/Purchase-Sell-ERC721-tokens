...
import "./raunakCoin.sol";


contract RNFT is Ownable
{
    // The price to create new RNFT
    uint256 public _mintingPrice;

    // The currency to create new RNFT
    Rcoin public _mintingCurrency;

    mapping(uint256 => bytes3) tokens;
    mapping(uint256 => address) owners;

    // map for sell tokens from tokenId => tokenPrice;
    ...
    function mintingCurrency() external view returns (Rcoin) {
    ...
    function setMintingCurrency(Rcoin newMintingCurrency) onlyOwner external {
    ...
    function purchaseToken(address _addr, uint256 tokenId) public {

        Rcoin rcoin = Rcoin(_addr);

        uint256 tokenPrice = forSale[tokenId];
        require(tokenPrice != 0, "the token is not for sale");
        require(
            rcoin.balanceOf(msg.sender) >= tokenPrice,
        ...
        // return rcoin.balanceOf(msg.sender);
        rcoin.approve(address(this), tokenPrice);
        rcoin.transferFrom(msg.sender, owners[tokenId], tokenPrice);
    }
